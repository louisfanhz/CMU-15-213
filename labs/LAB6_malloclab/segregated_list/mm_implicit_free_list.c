/*
 * mm.c - implements malloc, realloc, free and init using
 *        implicit free list. To find a fit for allocating
 *        new blocks, we need to traverse the list until
 *        the first fit is found
 * 
 */
#include <stdio.h>
#include <stdlib.h>
...
/* The following build uses implicit free list */
...
/* 
 * mm_malloc - traverse the entire free list to find suitable place
...
static void place(void *bp, size_t asize)
{
    ...
    if ((bsize-asize) >= (2*DSIZE)) { /* remainder >= minimum size */
